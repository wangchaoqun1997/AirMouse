...
void deque<_TypeT, _Allocator>::_C_alloc_at_begin ()
{
    ...
    if (!empty ()) {
        if (_C_begin._C_node == _C_map) {
            ...
            _TRY {
                __tmp = __alloc.allocate (new_map_size + 1, _C_map);
            }
            _CATCH (...) {
                _RWSTD_VALUE_ALLOC (_C_value_alloc_type,
            ...
            __alloc.deallocate (_C_map, _C_map_size + 1);
            _C_map = __tmp;
        ...
        size_type new_map_size = _C_bufsize ();
        if (new_map_size >= 2)
            new_map_size--;
        _TRY {
            _C_map = _C_map_alloc_type (*this).allocate (new_map_size + 1,
                                                               _C_map);
        }
        _CATCH (...) {
            _RWSTD_VALUE_ALLOC (_C_value_alloc_type,
...
void deque<_TypeT, _Allocator>::_C_alloc_at_end ()
{
    ...
    if (!empty ()) {
        ...
        _TRY {
            _C_map =
                _C_map_alloc_type (*this).allocate (new_map_size + 1, _C_map);
        }
        _CATCH (...) {
            _RWSTD_VALUE_ALLOC (_C_value_alloc_type,
                                deallocate (__ptr, _C_bufsize ()));
...
void deque<_TypeT, _Allocator>::_C_free_at_begin ()
{
    ...
    if (empty ()) {
        ...
        _C_map_alloc_type (*this).deallocate (_C_map, _C_map_size + 1);
    }
    else
