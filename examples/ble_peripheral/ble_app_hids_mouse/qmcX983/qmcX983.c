...
{
	I2C1_Read_Addr8(QMCX983_ADDRESS,*data,data,length);
}
static int8_t I2C_TxData(uint8_t *data,uint8_t length)
{
	I2C1_Write_Addr8(QMCX983_ADDRESS,*data,*(data+1));
}
static void qmcX983_fix(void)
{
...
static void qmcX983_start_measure()
{
...
void read_qmcX983_xyz(int *data)
{
	int8_t rdy=0,t1=0,res;
	uint8_t databuf[6]={0},mag_data[6]={0};
	int output[3]={0};
	int hw_d[3] = {0};
	/* Check status register for data availability */
	while(!(rdy & 0x07) && t1<3){
		databuf[0]=STA_REG_ONE;
		I2C_RxData(databuf,1);
		rdy=databuf[0];
	...
	I2C_RxData(databuf, 6);
	for(int i=0;i<6;i++){
		mag_data[i]=databuf[i];
		//MSE_LOG("QMCX983 read mag_xyz [%d]\n",databuf[i]);
	}
	...
	hw_d[0] = hw_d[0] * 1000 /xy_sensitivity;
	hw_d[1] = hw_d[1] * 1000 /xy_sensitivity;
	hw_d[2] = hw_d[2] * 1000 /z_sensitivity;
...
//static struct i2c_board_info __initdata i2c_qmcX983={ I2C_BOARD_INFO("qmcX983", (0X2d))}; // 0x2c
...
#define MAG_FILTER_ZK
#ifdef MAG_FILTER_ZK

static int hw_d_LAST[3] = {0};

#define MAX_FIR_LENGTH 20 

struct data_filter
{ 
     int rawx[MAX_FIR_LENGTH];
     int rawy[MAX_FIR_LENGTH];
     int rawz[MAX_FIR_LENGTH];
...
struct data_filter   mag_raw_filter;  
...
static DEFINE_MUTEX(qmc_i2c_mutex);
...
static int I2C_RxData(char *rxData, int length)
{
	...
	//this_client->timing=400;//zk.xu add
...
*/
	...
	{
		int idx; 
		if (mag_raw_filter.num < MAX_FIR_LENGTH) {
				mag_raw_filter.rawx[mag_raw_filter.num] =hw_d[0];
				mag_raw_filter.sumx += hw_d[0];

				mag_raw_filter.rawy[mag_raw_filter.num] =hw_d[1];
				mag_raw_filter.sumy += hw_d[1];

				mag_raw_filter.rawz[mag_raw_filter.num] =hw_d[2];
				mag_raw_filter.sumz += hw_d[2];
		 ...
		 {
				int tmpx=mag_raw_filter.sumx/MAX_FIR_LENGTH;
				int tmpy=mag_raw_filter.sumy/MAX_FIR_LENGTH;
				int tmpz=mag_raw_filter.sumz/MAX_FIR_LENGTH;
				 

	            unsigned int magP=(hw_d[0]-tmpx)*(hw_d[0]-tmpx)+ \
					((hw_d[1]-tmpy)*(hw_d[1]-tmpy))+((hw_d[2]-tmpz)*(hw_d[2]-tmpz));

				idx = mag_raw_filter.idx % MAX_FIR_LENGTH;
				
				mag_raw_filter.sumx -= mag_raw_filter.rawx[idx];
				mag_raw_filter.rawx[idx] =hw_d[0];
				mag_raw_filter.sumx +=hw_d[0];


				mag_raw_filter.sumy -= mag_raw_filter.rawy[idx];
				mag_raw_filter.rawy[idx] =hw_d[1];
				mag_raw_filter.sumy +=hw_d[1];

				mag_raw_filter.sumz -= mag_raw_filter.rawz[idx];
				mag_raw_filter.rawz[idx] =hw_d[2];
				mag_raw_filter.sumz +=hw_d[2];

				mag_raw_filter.idx++;

		 		if(magP>3000000){/*TOBe check the value*/
					hw_d[0]=hw_d_LAST[0];
					hw_d[1]=hw_d_LAST[1];
					hw_d[2]=hw_d_LAST[2];
					printk(" error data---------\n");
...
/*------------------------------for daemon------------------------*/
	...
	case MSENSOR_IOCTL_READ_FACTORY_SENSORDATA:
		...
		printk("xzk MSENSOR_IOCTL_READ_FACTORY_SENSORDATA\n");
...
static int qmcX983_m_enable(int en)
{
	...
	if(en == 1)
	{
		atomic_set(&m_flag, 1);
		v_open_flag |= 0x01;
		qmcX983_enable(this_client);// add
	}
	else
	{
		atomic_set(&m_flag, 0);
	...
	if(v_open_flag==0)
	 ...
	 memset(hw_d_LAST, 0, sizeof(int)*3);
#endif
...
static int qmcX983_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//data->hw = get_cust_mag_hw(); // mt97 change
